...
static int stmpe_devices_init(struct stmpe *stmpe)
{
	...
	for (i = 0; i < variant->num_blocks; i++) {
		...
		printk("platform_blocks:%d",platform_blocks);

		printk("block->block:%d",block->block);
			   ...
			   struct device_node *np)
{
	...
	printk("pdata->irq_gpio:%d",pdata->irq_gpio);
	...
	else pdata->irq_trigger = IRQF_TRIGGER_NONE;
