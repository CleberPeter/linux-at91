...
static int trf7970a_init(struct trf7970a *trf)
{
	...
	ret = trf7970a_cmd(trf, TRF7970A_CMD_SOFT_INIT); // 0x83
	if (ret)
		goto err_out;

	ret = trf7970a_cmd(trf, TRF7970A_CMD_IDLE); // 0x80
	if (ret)
		goto err_out;
	...
	ret = trf7970a_write(trf, TRF7970A_MODULATOR_SYS_CLK_CTRL,
			     trf->modulator_sys_clk_ctrl); // 0x09
	if (ret)
		goto err_out;

	ret = trf7970a_write(trf, TRF7970A_REG_IO_CTRL,
			     trf->io_ctrl | TRF7970A_REG_IO_CTRL_VRS(0x1)); // 0x0B
	if (ret)
		goto err_out;

	ret = trf7970a_write(trf, TRF7970A_NFC_TARGET_LEVEL, 0); // 0x18
	if (ret)
		goto err_out;
	...
	ret = trf7970a_write(trf, TRF7970A_ADJUTABLE_FIFO_IRQ_LEVELS,
			     ...
			     TRF7970A_ADJUTABLE_FIFO_IRQ_LEVELS_WLL_32); // 0x14
	if (ret)
		goto err_out;

	ret = trf7970a_write(trf, TRF7970A_SPECIAL_FCN_REG1, 0); //0x10
	if (ret)
		goto err_out;
...
static int trf7970a_power_up(struct trf7970a *trf)
{
	...
	if (trf->en_gpiod) gpiod_set_value_cansleep(trf->en_gpiod, 1);
...
static int trf7970a_power_down(struct trf7970a *trf)
{
	...
	if (trf->en_gpiod) gpiod_set_value_cansleep(trf->en_gpiod, 0);
...
static int trf7970a_probe(struct spi_device *spi)
{
	...
	/* On ZEUS rfid adapter EN pin is always in 1 */
	trf->en_gpiod = devm_gpiod_get_index_optional(trf->dev, "ti,enable", 0,
					     GPIOD_OUT_LOW);
	
	if (!trf->en_gpiod) {
		dev_info(trf->dev, "No EN GPIO property\n");
	} else if (IS_ERR(trf->en_gpiod)) {
